    ...
    Composite {
        polys: self.polys
            ...
            .map(|x| {
                ...
                let new_coeffs = MultivariatePoly::partial_evaluate(&x.coeffs, index, value[0]);
...
pub fn evaluate(&self, values: &Vec<Option<F>>) -> F {
    ...
    // Print the values and number of variables
    // println!("Values: {:?}", values);
    // println!("Number of variables: {}", self.polys[0].num_vars);
...
fn test_partial_evaluate_new_example() {
  // (a + b) * (b + ab) + (ab + b + a)
  let poly_a = vec![0, 1, 1, 2].iter().map(|x| Fq::from(x.clone())).collect();
  let poly_b = vec![0, 1, 0, 2].iter().map(|x| Fq::from(x.clone())).collect();
  let poly_c = vec![0, 1, 1, 3].iter().map(|x| Fq::from(x.clone())).collect();
...
#[test]
fn test_partial_evaluate_another_new_example() {
  // (a + 2b) * (3b + 4ab) + (5ab + 6b + 7a)
  let poly_a = vec![0, 2, 1, 3].iter().map(|x| Fq::from(x.clone())).collect();
  let poly_b = vec![0, 3, 0, 7].iter().map(|x| Fq::from(x.clone())).collect();
  let poly_c = vec![0, 6, 7, 18].iter().map(|x| Fq::from(x.clone())).collect();
